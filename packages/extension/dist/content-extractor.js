...
exports.showExtractionFailureNotification = showExtractionFailureNotification;
 ...
 */
function extractSustainabilityClaims(doc) {
    const claims = [];
 ...
 */
function extractCertificationMentions(doc) {
    const mentions = [];
 ...
 */
function extractCurrency(doc, priceText) {
...
function extractCategory(doc) {
    ...
    const breadcrumb = doc.querySelector('[class*="breadcrumb"], nav[aria-label="breadcrumb"]');
    if (breadcrumb) {
...
function extractImageUrl(doc) {
    for (const selector of IMAGE_SELECTORS) {
        ...
        if (el instanceof HTMLMetaElement) {
            const content = el.getAttribute("content");
            if (content?.trim())
                return upgradeImageUrl(content.trim());
        }
            ...
            if (src?.trim() && !src.startsWith("data:"))
                return upgradeImageUrl(src.trim());
        }
 ...
 */
function showExtractionFailureNotification() {
    const message = "EcoPulse could not extract product information from this page. " +
...
exports.ContentExtractor = { extract };
