 ...
 */
function parseSegment(segment: string): FabricComponent | null {
  const trimmed = segment.trim();
  if (!trimmed) return null;

  // Match percentage (before or after material name)
  const percentBeforeMatch = trimmed.match(
    /^(\d+(?:\.\d+)?)\s*%\s*(.+)$/i
  );
  const percentAfterMatch = trimmed.match(
    /^(.+?)\s+(\d+(?:\.\d+)?)\s*%$/i
  );

  let percentage = 0;
  let materialPart: string;
  ...
  const component: FabricComponent = {
 ...
 */
function extractQualifierAndMaterial(text: string): {
  material: string;
 ...
 */
function parseBlendText(text: string): FabricComponent[] {
  const lower = text.toLowerCase();
  const cleaned = BLEND_KEYWORDS.reduce(
    (t, kw) => t.replace(new RegExp(`\\b${kw}\\b`, "gi"), ""),
    lower
  ).trim();
 ...
 *   → "60% Organic Cotton"
 */
export function format(components: FabricComponent[]): string {
