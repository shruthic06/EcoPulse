...
export interface LLMProvider {
  generate(prompt: string): Promise<{ text: string; confidence: "high" | "medium" | "low" }>;
}
...
const FABRIC_KNOWLEDGE: Record<string, { info: string; confidence: "high" | "medium" | "low" }> = {
  cotton: {
...
const CHEMICAL_KNOWLEDGE: Record<string, { info: string; confidence: "high" | "medium" | "low" }> = {
  formaldehyde: {
...
class DefaultLLMProvider implements LLMProvider {
  async generate(prompt: string): Promise<{ text: string; confidence: "high" | "medium" | "low" }> {
...
let llmProvider: LLMProvider = new DefaultLLMProvider();
 ...
 * Requirements: 6.2
 */
export async function analyzeMaterialComposition(
  userId: string,
  ...
  const parseResult = parse(compositionText);
  ...
  const qualifiers = parseResult.components
    ...
    .map((c) => c.qualifier!);
  const certResults = verify(qualifiers);
