...
const GREENWASHING_PENALTY = {
    high: 25,
    medium: 15,
    low: 10,
};
/** Severity-based penalty for chemical risks. */
const CHEMICAL_RISK_PENALTY = {
    high: 20,
    medium: 12,
    low: 5,
};
 ...
 */
function computeEnvironmentalScore(fabricComponents, certificationResults) {
 ...
 * - Empty fabric components → default low score of 20
 */
function computeHealthScore(chemicalRisks, fabricComponents) {
    if (fabricComponents.length === 0) {
        return 20;
    }
    let score = 80;
 ...
 */
function computeGreenwashingScore(greenwashingSignals) {
    let score = 100;
...
function computeScore(input) {
    const environmental = computeEnvironmentalScore(input.fabricComponents, input.certificationResults);
    const health = computeHealthScore(input.chemicalRisks, input.fabricComponents);
    const greenwashing = computeGreenwashingScore(input.greenwashingSignals);
    const overallIndicator = environmental >= 50 && health >= 50 && greenwashing >= 50
        ? "green"
        : "red";
